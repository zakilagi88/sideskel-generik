...
namespace App\Filament\Clusters\Kependudukan\Resources\KartuKeluargaResource\Pages;

use App\Enums\Kependudukan\StatusDasarType;
use App\Filament\Clusters\Kependudukan\Resources\KartuKeluargaResource;
                                            ...
                                            Select::make('kk_id')
                                                ...
                                                ->options(
                                                    ...
                                                    KartuKeluarga::with('kepalaKeluarga')
                                                        ->when($get('dalam_luar_wilayah') === 'Dalam', fn ($query) => $query->where('wilayah_id', auth()->user()->wilayah_id)->orderBy('wilayah_id', 'asc'))
                                                        ->when($get('dalam_luar_wilayah') === 'Luar', fn ($query) => $query->where('wilayah_id', '!=', auth()->user()->wilayah_id)->orderBy('wilayah_id', 'asc'))
                                                        ->get()
                                                        ->map(function ($item) {
                                                            return [
                                                                'key' => $item->kk_id,
