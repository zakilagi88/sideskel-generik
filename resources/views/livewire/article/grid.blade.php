                        ...
                        <div class="bg-transparent dark:bg-gray-800 dark:text-gray-50">
                            <div class="container grid grid-cols-12 mx-auto dark:bg-gray-900">
                                <div class="bg-no-repeat bg-cover dark:bg-gray-700 col-span-full lg:col-span-4"
                                    style="background-image: url('{{ asset('storage/' . $article->featured_image_url) }}');
                        ...
                        <article class="flex flex-col ">
                            <div>
                                <img src="{{ asset('storage/' . $article->featured_image_url) }}"
                                    alt="{{ $article->slug }}" class="w-full h-64 object-cover rounded-lg mb-3">
                            </div>
                            <div>
                                <div class="flex items-center gap-x-4 text-xs">
                                    <time datetime="2020-03-16"
                                        class="text-gray-500">{{ $article->published_at->format('F j, Y') }}</time>
                                    <a href="{{ route('category', $article->category) }}"
                                        class="relative z-10 rounded-full bg-gray-50 px-3 py-1.5 font-medium text-gray-600 hover:bg-gray-100">{{ str($article->category->name)->ucFirst() }}</a>
                                </div>
                            </div>
                            <div>
                            ...
                            <div class="flex flex-col">
                                <div class="text-gray-600 space-y-3">
                                    <div class="relative mt-3 flex items-center gap-x-4">
                                        <img src="{{ asset($article->user->getFilamentAvatarUrl()) }}" alt=""
